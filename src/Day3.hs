...
data Dir = GoUp | GoDown | GoLeft | GoRight
    deriving (Show, Eq)
...
check :: IO ()
check = do
    print $ "toMotion: " ++ show (all (== True) [ toMotion "R10" == Motion GoRight 10
                                                , toMotion "D5" == Motion GoDown 5
                                                , toMotion "L2" == Motion GoLeft 2
                                                , toMotion "U1" == Motion GoUp 1
                                                , not $ null (toMotion <$> T.splitOn "," cable1Data)])

    print $ "renderMotion: " ++ show (all (== True) [ renderMotion  (Motion GoRight 1) == [(1,0)]
                                                    , renderMotion  (Motion GoUp 2) == [(0,1), (0,1)]
                                                    ])

toMotion :: T.Text -> Motion
toMotion t
  | "U" `T.isPrefixOf` t = Motion GoUp $ getNum t
  | "D" `T.isPrefixOf` t = Motion GoDown $ getNum t
  | "L" `T.isPrefixOf` t = Motion GoLeft $ getNum t
  | "R" `T.isPrefixOf` t = Motion GoRight $ getNum t
    where
        getNum = read . T.unpack . T.tail



-- Constructs a sequence of unit vectors representing the motion:
-- >> renderMotion (Motion GoUp 2) == [(0,1), (0,1)]

renderMotion :: Motion -> [Coord]
renderMotion (Motion _ 0) = []
renderMotion (Motion GoUp n) = (0, 1) : renderMotion (Motion GoUp (n-1))
renderMotion (Motion GoDown n) = (0, -1) : renderMotion (Motion GoDown (n-1))
renderMotion (Motion GoLeft n) = (-1, 0) : renderMotion (Motion GoLeft (n-1))
renderMotion (Motion GoRight n) = (1, 0) : renderMotion (Motion GoRight (n-1))


-- Renders actual coordinates from a list of unit vector motions
-- >> renderCoords (0,0) . renderMotion $ [Motion GoRight 3] == [(0,1), (0,2), (0,3)]

renderCoords :: Coord -> [Coord] -> [Coord]
renderCoords _ [] = []
renderCoords (x,y) ((dx,dy):ds) = newCoord : renderCoords newCoord ds
    where
        newCoord = (x+dx,y+dy)

getIntersections :: [Coord] -> [Coord] -> [Coord]
getIntersections c1 c2 = S.toList $ s1 `S.intersection` s2
    where
        s1 = S.fromList c1
        s2 = S.fromList c2
